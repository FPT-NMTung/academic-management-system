export const ErrorCodeApi = {
  'room-error-0001': 'Cơ sở không tồn tại',
  'room-error-0002': 'Không tìm thấy phòng nào',
  'room-error-0003': 'Phòng đã tồn tại',
  'room-error-0004': '',
  'room-error-0005': '',
  'room-error-0006': '',
  'room-error-0007': '',
  'room-error-0008': 'Tên phòng phải có độ dài từ 2 trở lên',
  'room-error-0009': '',
  'room-error-0010': '',
  'room-error-0011': '',
  'room-error-0012': '',
  'room-error-0013': '',
  'room-error-0014': '',
  'room-error-0015': 'Phòng đã tồn tại trong hệ thống',

  'center-error-0001': '',
  'center-error-0002': '',
  'center-error-0003': '',
  'center-error-0004': '',
  'center-error-0005': '',

  'sro-error-0001': '',
  'sro-error-0002': '',
  'sro-error-0003': '',
  'sro-error-0004': '',
  'sro-error-0005': 'Số điện thoại đã tồn tại trong hệ thống',
  'sro-error-0006': 'Email cá nhân này đã tồn tại trong hệ thống',
  'sro-error-0007': 'Email tổ chức này đã tồn tại trong hệ thống',
  'sro-error-0008': '',
  'sro-error-0009': '',
  'sro-error-0010': '',
  'sro-error-0011': '',
  'sro-error-0012': 'Số CMND/CCCD đã tồn tại trong hệ thống',
  'sro-error-0013': '',
  'sro-error-0014': '',
  'sro-error-0015': '',
  'sro-error-0016': '',
  'sro-error-0017': '',
  'sro-error-0018': '',
  'sro-error-0019': '',
  'sro-error-0020': '',
  'sro-error-0021': '',
  'sro-error-0021-1': 'Email cá nhân đã được sử dụng',
  'sro-error-0022': '',
  'sro-error-0022-1': 'Email tổ chức đã được sử dụng',
  'sro-error-0022-2': 'Email cá nhân và email tổ chức không được trùng nhau',

  'address-error-0001': '',

  'course-error-0001': '',
  'course-error-0002': '',
  'course-error-0003': '',
  'course-error-0004': '',
  'course-error-0005': '',
  'course-error-0006': '',
  'course-error-0007': '',
  'course-error-0008': 'Mã chương trình học đã tồn tại',
  'course-error-0009': 'Mã khóa học đã tồn tại',
  'course-error-0010': '',
  'course-error-0011': '',
  'course-error-0012': '',
  'course-error-0013': '',
  'course-error-0014': '',

  'teacher-error-0002': '',
  'teacher-error-0003': '',
  'teacher-error-0004': 'Trùng mã thuế thu nhập cá nhân',
  'teacher-error-0005': 'Số điện thoại không hợp lệ',
  'teacher-error-0006': 'Email cá nhân không hợp lệ',
  'teacher-error-0007': 'Email tổ chức không hợp lệ',
  'teacher-error-0008': 'Số CMND/CCCD phải có độ dài từ 9 hoặc 12',
  'teacher-error-0009': '',
  'teacher-error-0010': '',
  'teacher-error-0011': '',
  'teacher-error-0012': '',
  'teacher-error-0013': 'Số điện thoại đã tồn tại trong hệ thống',
  'teacher-error-0014': 'Email cá nhân này đã tồn tại trong hệ thống',
  'teacher-error-0015': 'Email tổ chức này đã tồn tại trong hệ thống',
  'teacher-error-0016': 'Số CMND/CCCD đã tồn tại trong hệ thống',
  'teacher-error-0017': '',
  'teacher-error-0018': '',
  'teacher-error-0052-2': 'Email cá nhân và email tổ chức không được trùng nhau',

  'module-error-00021': 'Môn học với mã khóa học, học kỳ đã tồn tại',

  'class-error-0070': 'Tên lớp học đã được sử dụng',
  'class-error-0069': 'Tên lớp học chứa ký tự đặc biệt | Các ký tự cho phép: ( ) _ - ',

  'student-class-error-0001': 'Đã có lỗi xảy ra khi lưu thông tin học viên 0001',
  'student-class-error-0002': 'Mã số của học viên đã tồn tại',
  'student-class-error-0003': 'Thông tin của học viên đã tồn tại trên tài khoản khác',
  'student-class-error-0004': 'Đã có lỗi xảy ra khi lưu thông tin học viên 0004',
  'student-class-error-0005': 'Lớp học không tồn tại',
  'student-class-error-0006': 'Đã có lỗi xảy ra khi lưu thông tin học viên 0006',
  'System.FormatException': 'Định dạng dữ liệu trong file không hợp lệ',
};
